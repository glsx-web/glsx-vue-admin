const Theme = (version = '2.4.0') => {
  this.default_color = '#409EFF'
  this.color = ''
  this.version = version
}
const change = (val, oldVal) => {
  if (typeof val !== 'string') return
  const themeCluster = getThemeCluster(val.replace('#', ''))
  const originalCluster = getThemeCluster(oldVal.replace('#', ''))
  const getHandler = (variable, id) => {
    return () => {
      const originalCluster = getThemeCluster(this.default_color.replace('#', ''))
  ...
  if (!this.chalk) {
    const url = `https://cdn.bootcss.com/element-ui/${this.version}/theme-chalk/index.css`
